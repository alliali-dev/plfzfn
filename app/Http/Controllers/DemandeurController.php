...
class DemandeurController extends Controller
{
    ...
    { // enregistre une demandeurs
        ...
        if ($request->file('piece_demandeur')) {
            //stocker le fichier dans le dossier 

            $piece = $request->file('piece_demandeur');
            $piecename = Str::slug('piece') . '_' . str_replace(" ", "_", $request->nom) . '_' . time() . '_' . Str::random(10) . '.' . $piece->getClientOriginalExtension();
            $path = public_path('assets/images/pieces');
            if (!File::isDirectory($path)) {
                File::makeDirectory($path, 0777, true, true);
            }
            $piece->move($path, $piecename);
        }
